...
def analyze_attack(model_name, wrapper, baseline_data, adv_path, feature_names):
    ...
    xai_save = os.path.join(comparisons_dir, f"{model_name.lower().replace(' ', '_')}_xai_analysis.png")
    plot_xai_analysis(adv_attr, perturbation, feature_names, model_name, xai_save)
    comp_save = os.path.join(comparisons_dir, f"{model_name.lower().replace(' ', '_')}_attribution_comparison.png")
    plot_comparison(baseline_attr, adv_attr, feature_names, model_name, comp_save)
...
if __name__ == "__main__":
    ...
    attack_configs = [
        ("ACT PGD Energy", ACTEnergyWrapper, load_act,
         os.path.join(OUTPUT_DIR, 'act', 'act_pgd_energy_best_input.npy')),
        ("ACT PGD Latency", ACTLatencyWrapper, load_act,
         os.path.join(OUTPUT_DIR, 'act', 'act_pgd_latency_best_input.npy')),
        ("DeepAR PGD Energy", DeepAREnergyWrapper, lambda **kw: load_deepar(**kw)[0],
         os.path.join(OUTPUT_DIR, 'deepar', 'deepar_pgd_energy_best_input.npy')),
        ("DeepAR PGD Latency", DeepAREnergyWrapper, lambda **kw: load_deepar(**kw)[0],
         os.path.join(OUTPUT_DIR, 'deepar', 'deepar_pgd_latency_best_input.npy')),
    ]
    ...
    if len(results) > 1:
        ...
        plt.tight_layout()
        save_path = os.path.join(comparisons_dir, 'xai_cross_model_comparison.png')
        plt.savefig(save_path, dpi=150)
        print(f"Saved: {save_path}")
        plt.close()
