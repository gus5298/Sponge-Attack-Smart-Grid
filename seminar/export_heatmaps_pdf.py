...
Usage:
    python export_heatmaps_pdf.py --metric latency
    python export_heatmaps_pdf.py --metric energy
"""
...
def main():
    ...
    for model_name, path in cfg['models'].items():
        ...
        output_filename = f"{safe_name}{cfg['file_suffix']}"
    ...
    for model_name in cfg['models']:
        safe_name = model_name.lower().replace('-', '_').replace(' ', '_')
        print(f"  - {safe_name}{cfg['file_suffix']}")
