...
def run_shap_analysis(model, model_name, proxy_fn, baseline_data, adv_data, feature_names):
    ...
    plt.tight_layout()
    save_path = os.path.join(comparisons_dir, f"{model_name.lower().replace(' ', '_').replace('-', '_')}_shap_analysis.png")
...
def run_transferability_analysis(act_model, deepar_model, baseline_data,
    ...
    plt.tight_layout()
    save_path = os.path.join(comparisons_dir, 'attack_transferability_analysis.png')
    plt.savefig(save_path, dpi=150)
    print(f"\nSaved: {save_path}")
    plt.close()
    return results
...
def run_frequency_analysis(baseline_data, adv_inputs, feature_names):
    ...
    plt.tight_layout()
    save_path = os.path.join(comparisons_dir, 'frequency_analysis.png')
    plt.savefig(save_path, dpi=150)
    print(f"\nSaved: {save_path}")
    plt.close()
    ...
    plt.tight_layout()
    save_path2 = os.path.join(comparisons_dir, 'frequency_2d_fft.png')
...
if __name__ == "__main__":
    ...
    adv_inputs = load_adversarial_inputs({
        ...
        'DeepAR PGD Latency': os.path.join(OUTPUT_DIR, 'deepar', 'deepar_pgd_latency_best_input.npy'),
    })
