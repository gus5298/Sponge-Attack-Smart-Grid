...
from config import CONTEXT_LEN, PREDICTION_LEN, NUM_FEATURES, OUTPUT_DIR
...
deepar_bitflip_path = os.path.join(OUTPUT_DIR, 'deepar', 'deepar_bitflip_whitebox_best_input.npy')
if os.path.exists(deepar_bitflip_path):
    deepar_adv = np.load(deepar_bitflip_path).reshape(CONTEXT_LEN, NUM_FEATURES)
else:
...
chronos_baseline, _, _ = load_seed()

chronos_latency_path = os.path.join(OUTPUT_DIR, 'chronos', 'chronos_latency_best_input.npy')
if os.path.exists(chronos_latency_path):
    chronos_adv = np.load(chronos_latency_path).reshape(CONTEXT_LEN, NUM_FEATURES)
else:
...
output_path = os.path.join(comparisons_dir, 'energy_power_comparison.png')
